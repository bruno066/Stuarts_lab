...
class Scope(object):
    def __init__(self, chan, host, fold=19277, nmax=100,NORM=True,sequence=False):
        ...
        HORIZ_VAL = 0.05
        self.ax0 = axes([0.125+HORIZ_VAL,0.25,0.25,0.62])
        self.ax1 = axes([0.405+HORIZ_VAL,0.25,0.25,0.62])
        self.ax2 = axes([0.685+HORIZ_VAL,0.25,0.25,0.62])
        for i in range(len(self.channel)):
            if not self.NORM:
        ...
        self.axh0 = axes([0.125+HORIZ_VAL,0.05,0.25,0.15])
        self.axh1 = axes([0.405+HORIZ_VAL,0.05,0.25,0.15])
        self.axh2 = axes([0.685+HORIZ_VAL,0.05,0.25,0.15])
        ...
        self.remove_len10_sliderax = axes([0.125+HORIZ_VAL,0.91,0.25,0.02])
        self.remove_len11_sliderax = axes([0.405+HORIZ_VAL,0.91,0.25,0.02])
        self.remove_len12_sliderax = axes([0.685+HORIZ_VAL,0.91,0.25,0.02])
        for i in range(len(self.channel)):
        ...
        self.remove_len20_sliderax = axes([0.125+HORIZ_VAL,0.88,0.25,0.02])
        self.remove_len21_sliderax = axes([0.405+HORIZ_VAL,0.88,0.25,0.02])
        self.remove_len22_sliderax = axes([0.685+HORIZ_VAL,0.88,0.25,0.02])
        for i in range(len(self.channel)):
        ...
        # create 'shear' slider
        self.shear_sliderax = axes([0.15,0.96,0.8,0.02])
        ...
        mpl.pyplot.text(-0.68,7+VERT_VAL,'Sequence mode:',fontsize=18,fontproperties=font1)
        mpl.pyplot.text(-0.68,3+VERT_VAL,'"b" to toggle it then:\n      "y" to save\n      "n" to next',fontsize=18)
        mpl.pyplot.text(-0.68,-32+VERT_VAL,'Useful keys:',fontsize=18,fontproperties=font1)
        mpl.pyplot.text(-0.68,-40+VERT_VAL,'"c" to change colormap\n "v" to change vertical\n      /colorscale\n " " to pause\n "S" to save trace\n       and picture\n "q" to exit',fontsize=18)
...
if __name__=='__main__':
    ...
    if len(args) == 0:
        ...
        if len(temp_chan) == 1:
            print "\nYou must enter 3 channels separated with comas\n"
        ...
        print "You must enter 3 channels separated with comas"
    print chan
